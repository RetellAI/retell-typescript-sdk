...
await client.asset.create({ file: fs.createReadStream('/path/to/file') });

// Or if you have the web `File` API you can pass a `File` instance:
await client.asset.create({ file: new File(['my bytes'], 'file') });

// You can also pass a `fetch` `Response`:
await client.asset.create({ file: await fetch('https://somesite/file') });

// Finally, if none of the above are convenient, you can use our `toFile` helper:
await client.asset.create({ file: await toFile(Buffer.from('my bytes'), 'file') });
await client.asset.create({ file: await toFile(new Uint8Array([0, 1, 2]), 'file') });
```
