...
The current visible workflows do not show staging pushes overwriting production repo files. `ci.yml` builds the SDK and uploads npm tarballs to `pkg.stainless.com` when running in `RetellAI/retell-typescript-sdk-staging`; it does not unpack artifacts back into `RetellAI/retell-typescript-sdk`.

The remaining preservation risk is the automated production release branch. `stlc-auto-merge.yml` can merge a same-repo branch named `stainless-release`, but it does not create that branch. Before implementation, confirm how the current `stainless-release` branch is produced and whether it can delete production-only files.

If the current release branch is generated by applying artifact changes onto production `main`, no extra `keep_files` entry is needed for MCP deploy files. If the release branch is produced from a generated/staging tree that can replace production-only files, preserve these files through the active promotion mechanism:

- `.github/workflows/deploy-mcp-server.yml`
- `ecs/mcp/task-def-mcp-server.json`

Depending on the active pipeline, preservation may mean a promotion-script preserve list or a Stainless/stls `keep_files` entry in `RetellAI/docs`. Do not add `keep_files` blindly without confirming that it is still the active mechanism.
...
1. Verify how `stainless-release` is produced and whether production-only deploy files need preservation.
...
- Production-only deploy files could be deleted by a future generated release PR if that PR is created from a generated/staging tree rather than by applying artifact changes onto production `main`. Verify the release branch source before adding deploy files.
