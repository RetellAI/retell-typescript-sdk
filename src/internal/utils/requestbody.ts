...
const serializeContentType = (contentType: string, reqBody: any): [Record<string, any>, any] => {
    ...
    switch (contentType) {
        ...
        case "application/json":
            ...
            break;
        case "text/json":
            [requestHeaders, requestBody] = [{ "Content-Type": `${contentType}` }, reqBody];
            break;
...
const encodeFormUrlEncodedData = (data: any): string => {
    const fieldNames: string[] = Object.getOwnPropertyNames(data);

    let urlencoded = "";
    let amp = "";
    const appendPair = (key: string, value: string) => {
        urlencoded += `${amp}${encodeURIComponent(key)}=${encodeURIComponent(value)}`;
        amp = "&";
    };
    ...
    if (isNumberRecord(data) || isBooleanRecord(data) || isStringRecord(data)) {
        ...
        fieldNames.forEach((fname) => {
            ...
            if (formDecorator.JSON) {
                ...
                const val = JSON.stringify(data[fname]);
                appendPair(name, val);
                ...
                Object.keys(parsed).forEach((key) => {
                    parsed[key].forEach((v) => appendPair(key, v));
                });
                return;
            }
        });
...
function parseFormDecorator(formAnn: string): FormDecorator {
    const formDecorator: FormDecorator = new FormDecorator("", "form", false, false);
...
class FormDecorator {
    ...
    constructor(Name?: string, Style?: string, Explode?: boolean, JSON?: boolean) {
        this.Name = Name;
        this.Style = Style;
...
function encodeMultipartFormData(form: any): FormData {
    ...
    fieldNames.forEach((fname) => {
        const mpFormAnn: string = Reflect.getMetadata(mpFormMetadataKey, form, fname);

        if (mpFormAnn == null) return;

        const mpFormDecorator: MultipartFormDecorator = parseMultipartFormDecorator(mpFormAnn);

        if (mpFormDecorator.File) return encodeMultipartFormDataFile(formData, form[fname]);
...
function encodeMultipartFormDataFile(formData: FormData, file: any): FormData {
    ...
    fieldNames.forEach((fname) => {
        const mpFormAnn: string = Reflect.getMetadata(mpFormMetadataKey, file, fname);
...
function parseMultipartFormDecorator(mpFormAnn: string): MultipartFormDecorator {
    // example "name=file"
...
function parseRequestDecorator(requestAnn: string): RequestDecorator {
    ...
    const requestDecorator: RequestDecorator = new RequestDecorator("application/octet-stream");
