            ...
            for (const prop of props) {
                if (payload && payload.hasOwnProperty(prop.key)) {
                    ...
                    if (isSpeakeasyBase(prop.type) && value != null) {
                        ...
                        (this as any)[prop.key] = handleArray(value, prop.elemType, prop.elemDepth);
...
>(params?: { data?: string; elemType?: { new (): T }; elemDepth?: number }): PropertyDecorator {
...
export function templateUrl(stringWithParams: string, params: Record<string, string>): string {
...
export function generateURL(
    ...
    fieldNames.forEach((fname) => {
        const requestBodyAnn: string = Reflect.getMetadata(requestMetadataKey, pathParams, fname);
        ...
        const ppDecorator: ParamDecorator = parseParamDecorator(ppAnn, fname, "simple", false);
...
export function isNumberRecord(obj: any): obj is Record<string, number> {
    ...
    return Object.getOwnPropertyNames(obj).every((prop) => typeof obj[prop] === "number");
}
...
export function isBooleanRecord(obj: any): obj is Record<string, boolean> {
    ...
    return Object.getOwnPropertyNames(obj).every((prop) => typeof obj[prop] === "boolean");
}
