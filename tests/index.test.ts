...
import { APIPromise } from 'retell-sdk/core/api-promise';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['RETELL_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: Retell) => {
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['RETELL_LOG'] = 'debug';
    const client = new Retell({ logger: logger, apiKey: 'YOUR_RETELL_API_KEY' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['RETELL_LOG'] = 'not a log level';
    const client = new Retell({ logger: logger, apiKey: 'YOUR_RETELL_API_KEY' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'RETELL_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Retell({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'YOUR_RETELL_API_KEY' });
    ...
    afterEach(() => {
      process.env['RETELL_BASE_URL'] = undefined;
    });
