...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new RetellAI({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['RETELL_AI_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new RetellAI({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['RETELL_AI_BASE_URL'] = 'https://example.com/from_env';
      const client = new RetellAI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['RETELL_AI_BASE_URL'] = ''; // empty
      const client = new RetellAI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.retellai.com');
    });

    test('blank env variable', () => {
      process.env['RETELL_AI_BASE_URL'] = '  '; // blank
      const client = new RetellAI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.retellai.com');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new RetellAI({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new RetellAI({ apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new RetellAI({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new RetellAI({ apiKey: 'My API Key', fetch: testFetch });
