...
describe('resource llm', () => {
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      retellSdk.llm.retrieve('16b980523634a6dc504898cda492e939', { path: '/_stainless_unknown_path' }),
  ...
  test('delete: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      retellSdk.llm.delete('oBeDLoLOeuAbiuaMFXRtDOLriTJ5tSxD', { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(RetellSdk.NotFoundError);
  });
});
